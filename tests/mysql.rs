...
/// Spin up a Mysql container and return an instrumented pool connected to it.
async fn test_pool() -> (Pool<MySql>, testcontainers::ContainerAsync<GenericImage>) {
    ...
    let raw = sqlx::MySqlPool::connect(&url).await.unwrap();
