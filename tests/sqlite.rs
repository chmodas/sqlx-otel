...
fn assert_common_span_attributes(span: &SpanData) {
...
fn assert_error_span(span: &SpanData) {
    assert!(
...
async fn execute_creates_span_via_pool() {
    ...
    assert_common_span_attributes(&spans[0]);
...
async fn fetch_optional_records_zero_rows() {
    ...
    assert_common_span_attributes(select_span);
    assert_eq!(
